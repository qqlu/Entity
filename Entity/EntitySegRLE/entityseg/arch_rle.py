...
__all__ = ["EntityFPNRLE"]

@META_ARCH_REGISTRY.register()
class EntityFPNRLE(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        valid = [True for i in range(B)]
        if self.training and "instances" in batched_inputs[0]:
            images  = self.preprocess_image(batched_inputs, valid)
            for i in range(B):
            ...
            for per_im_gt_inst in gt_instances:
                ...
                bitmasks = per_im_gt_inst.get("gt_masks")
                h, w = bitmasks.size()[1:]
                # pad to new size
    ...
    def preprocess_image(self, batched_inputs, valid):
        """
        Normalize, pad and batch the input images.
        """
        images = [x["image"].to(self.device) for i,x in enumerate(batched_inputs) if valid[i]==True]
        # pdb.set_trace()
    ...
    def postprocess(self, results, output_height, output_width, padded_im_h, padded_im_w, mask_threshold=0.5):
        ...
        if results.has("pred_global_masks"):
            # if output_height >=2000 and output_width >=2000:
            #     results.to("cpu")
        ...
        if output_height >=2000 and output_width >=2000:
            origin_masks = origin_masks.cpu()
        num_instances, H, W = origin_masks.shape
        filter_masks = []

        # initialize background
        if output_height >=2000 and output_width >=2000:
            mask_0 = torch.zeros((H, W)).cpu() + 0.001
        else:
