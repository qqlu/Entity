...
from . import simiaccess as simiUtils
import copy
...
class COCOeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm', openEva=False, simiPath=None):
        '''
        ...
        self.params = Params(iouType=iouType, openEva=openEva, simiPath=simiPath) # parameters
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        dtIg = np.zeros((T,D))
        if p.openEva:
            simiAccess = simiUtils.SIMIaccess(p.simiPath)
        if not len(ious)==0:
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    gtm[tind,m]     = d['id']
                    if p.openEva:
                        stm[tind,dind] = simiAccess.findSimi(dt_lb[dind], gt_lb[m])
    ...
    def accumulate(self, p = None):
        ...
        T           = len(p.iouThrs)
        R           = len(p.recThrs)
        K           = len(p.catIds) if p.useCats and not p.openEva else 1
        A           = len(p.areaRng)
        M           = len(p.maxDets)
...
class Params:
    ...
    '''
    def setDetParams(self, useCats=1, openEva=0, simiPath=None):
        self.imgIds = []
        self.catIds = []
    ...
    def __init__(self, iouType='segm', openEva=False, simiPath=None):
        if iouType == 'segm' or iouType == 'bbox':
            if openEva==True and simiPath is not None:
                self.setDetParams(self, useCats=0, openEva=1, simiPath=simiPath)
            else:
                self.setDetParams()
        elif iouType == 'keypoints':
            self.setKpParams()
